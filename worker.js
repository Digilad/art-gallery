...
const ARTWORK_CACHE_KEY = new Request('https://art-gallery-cache.invalid/api/artwork-v2');
...
async function meetsImageRequirements(imageUrl) {
...
async function fromMet(artists) {
  ...
  for (let attempt = 0; attempt < 10; attempt += 1) {
    ...
    if (object.primaryImage && object.isPublicDomain && object.objectName?.toLowerCase().includes('painting') && await meetsImageRequirements(object.primaryImage)) {
      return { source: 'met', title: object.title || 'Untitled', artist: object.artistDisplayName || object.culture || 'Unknown artist', year: object.objectDate || '', museum: object.repository || 'The Metropolitan Museum of Art', apiMethod: 'Met Collection API: GET /search and GET /objects/{objectID}', imageUrl: object.primaryImage };
    }
  }
...
async function fromArtInstitute(artists) {
  ...
  for (let attempt = 0; attempt < 10; attempt += 1) {
    ...
    const imageUrl = data?.image_id ? `https://www.artic.edu/iiif/2/${data.image_id}/full/1920,/0/default.jpg` : '';
    if (data?.image_id && data.is_public_domain && data.artwork_type_title?.toLowerCase().includes('painting') && await meetsImageRequirements(imageUrl)) {
      return { source: 'aic', title: data.title || 'Untitled', artist: data.artist_title || data.artist_display || 'Unknown artist', year: data.date_display || '', museum: 'Art Institute of Chicago', apiMethod: 'Art Institute of Chicago API: GET /artworks/search and GET /artworks/{id}', imageUrl };
    }
  }
...
async function artwork(request, env) {
  ...
  const excluded = new URL(request.url).searchParams.get('exclude');
  const providers = [{ source: 'met', load: fromMet }, { source: 'aic', load: fromArtInstitute }]
    .filter((provider) => provider.source !== excluded)
    .sort(() => Math.random() - 0.5);
  let lastError;
  for (const provider of providers) {
    try {
      const result = await provider.load(artists);
      const imageProxy = new URL('/api/image', request.url);
      imageProxy.searchParams.set('url', result.imageUrl);
      const body = { ...result, workerVersion: WORKER_VERSION, imageUrl: imageProxy.toString() };
...
export default {
  async fetch(request, env) {
    ...
    if (path === '/api/health') return json({ workerVersion: WORKER_VERSION, status: 'ok' });
